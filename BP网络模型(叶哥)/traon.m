function [layers_rec,weight_rec,theta_rec,maxtraacc]=traon(x_tra,y_tra,dimhidlay_in,maxrrr_in,epsilon_in)
%%
%初始化
x=x_tra(:,1);y=y_tra(:,1);
dimx=length(x);dimy=length(y);
dimhidlay=dimhidlay_in;%每层个数
theta0=0.1;weight0=0.1;eta=0.1;
...
weight=cell(1,1+numhidlay);
for i=1:length(weight)
    weight{i}=weight0*ones(length(layers{i+1}),length(layers{i}));
end
for i=1:length(weight)
    for j=1:length(layers{i+1})
        for k=1:length(layers{i})
         weight{i}(j,k)=weight{i}(j,k)+j*0.001+k*0.001;
        end
    end
end
